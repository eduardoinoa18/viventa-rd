...
function generateWhatsAppMessage(listing: Listing): string {
  const title = listing.displayTitle || listing.title || 'esta propiedad';
  ...
  return encodeURIComponent(
    `Hola VIVENTA, me interesa la siguiente propiedad:\n\n` +
    `📍 ${title}\n` +
    `💰 ${price}\n` +
    `📌 ${listing.address}\n\n` +
...
export default function WhatsAppFloatingCTA({
  ...
  const whatsappUrl = `https://wa.me/${VIVENTA_PHONE_E164}?text=${message}`;
  const callHref = `tel:+${VIVENTA_PHONE_E164}`;
  const mailHref = `mailto:${VIVENTA_EMAIL_DISPLAY}`;
    ...
    <>
      ...
      <div className={`lg:hidden fixed bottom-20 right-4 z-50 ${className}`}>
        {!showCard ? (
          <button
            ...
            className="w-16 h-16 bg-[#25D366] rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform"
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-4 w-72 relative">
            ...
            <div className="text-center mb-4 mt-2">
              {agent.photo ? (
                ...
                <div className="w-16 h-16 bg-gradient-to-br from-[#00A6A6] to-[#00C8C8] rounded-full mx-auto mb-2 flex items-center justify-center text-white text-2xl font-bold">
                  {agent.name.charAt(0)}
                </div>
              )}
              ...
              {agent.avgResponseTime && agent.avgResponseTime < 120 && (
                <p className="text-xs text-gray-600 mt-1">🟢 Responde en menos de 2 horas</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              <a
                ...
                className="w-full py-3 bg-[#25D366] text-white rounded-lg font-medium hover:bg-[#20BA5A] transition text-center flex items-center justify-center gap-2 shadow-md"
              >
      ...
      <div className={`hidden lg:block sticky top-24 ${className}`}>
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center">Horario de atención: Lun-Vie 9am-6pm</p>
          </div>
        </div>
      </div>
    </>
