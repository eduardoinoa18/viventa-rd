...
function DealCard({ deal, isDragging = false, onView }: DealCardProps) {
  ...
  return (
    <div
      ...
      `}
    >
      {/* Drag region — takes up the whole card */}
      <div className="cursor-grab touch-none p-3.5 active:cursor-grabbing">
        ...
        {(deal.unitId || deal.projectId) && (
          <p className="mt-0.5 truncate text-[11px] text-gray-400">
            {[deal.unitId, deal.projectId && `Proj: ${deal.projectId.slice(0, 8)}…`]
              .filter(Boolean)
              .join(' · ')}
          </p>
        )}
      ...
      {/* View button — no drag listeners, safe to click */}
      {onView && (
        <button
          type="button"
...
function KanbanColumn({ config, deals, isOver, onCardClick }: KanbanColumnProps) {
  ...
  return (
    <div className="flex min-w-[220px] max-w-[260px] flex-1 flex-col">
      ...
      <div
        ref={setNodeRef}
        className={`flex flex-1 flex-col gap-2 overflow-y-auto rounded-xl border-2 border-dashed p-2 transition-colors duration-150
          min-h-[120px]
      ...
      >
        ...
        {deals.length === 0 && (
          <div className="flex flex-1 items-center justify-center text-xs text-gray-400">No deals</div>
        )}
      </div>
    </div>
  )
...
export interface KanbanBoardProps {
  deals: TransactionRecord[]
  onStageChange: (dealId: string, newStage: TransactionStage) => Promise<void>
  onCardClick?: (deal: TransactionRecord) => void
  loading?: boolean
  error?: string | null
}
...
export default function KanbanBoard({ deals, onStageChange, onCardClick, loading, error }: KanbanBoardProps) {
  ...
  // Sync external deals changes (e.g. after re-fetch)
  useEffect(() => {
    setItems(deals)
  }, [deals])
  ...
  const grouped = useMemo(() => {
    const map: Record<TransactionStage, TransactionRecord[]> = {} as any
    TRANSACTION_STAGES.forEach((s) => { map[s] = [] })
  ...
  const handleDragEnd = useCallback(
    async (e: DragEndEvent) => {
      ...
      try {
        await onStageChange(draggedId, targetStage)
      } catch {
        // Rollback on error
        setItems((prev) =>
          prev.map((d) => (d.id === draggedId ? { ...d, stage: current.stage } : d)),
        )
      }
    },
    [items, onStageChange],
