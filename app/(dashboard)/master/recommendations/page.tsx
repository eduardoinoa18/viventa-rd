  ...
  function load(r: string) {
    ...
    fetch(`/api/admin/recommendations/overview?range=${r}`)
      .then(async (res) => {
        ...
        if (!res.ok || !json?.ok) throw new Error(json?.error || 'Error al cargar el pipeline de recomendaciones')
        setData(json.data)
      })
    ...
    <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-600 text-sm">Salud de la cola de jobs de recomendación y métricas de entrega de email</p>
        </div>
      ...
      {loading && !data && (
        <div className="mb-6 bg-white border border-gray-200 rounded-lg p-4 text-sm text-gray-500">
          Cargando datos del pipeline de recomendaciones...
        </div>
      )}
      ...
      {totals && (
        <section className="mb-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <FiClock className="text-[#00A676]" /> Estado de la cola de jobs
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="text-xs text-gray-500 mb-1">Total de jobs</div>
              <div className="text-2xl font-bold text-[#0B2545]">{totals.total}</div>
            </div>
            <div className="bg-white border border-blue-200 rounded-xl p-4">
              <div className="text-xs text-blue-600 mb-1 flex items-center gap-1"><FiClock /> En cola</div>
            ...
            <div className="bg-white border border-yellow-200 rounded-xl p-4">
              <div className="text-xs text-yellow-600 mb-1 flex items-center gap-1"><FiRefreshCw /> Procesando</div>
              <div className="text-2xl font-bold text-yellow-700">{totals.processing}</div>
            </div>
            <div className="bg-white border border-green-200 rounded-xl p-4">
              <div className="text-xs text-green-600 mb-1 flex items-center gap-1"><FiCheckCircle /> Enviados</div>
              <div className="text-2xl font-bold text-green-700">{totals.sent}</div>
            </div>
            <div className="bg-white border border-red-200 rounded-xl p-4">
              <div className="text-xs text-red-600 mb-1 flex items-center gap-1"><FiXCircle /> Fallidos</div>
              <div className="text-2xl font-bold text-red-700">{totals.failed}</div>
            </div>
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="text-xs text-gray-500 mb-1 flex items-center gap-1"><FiSkipForward /> Omitidos</div>
          ...
          {deliveryRate !== null && (
            <div className="mt-4 bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
              <div className="flex-1">
                <div className="text-xs text-gray-500 mb-1">Tasa de entrega (enviados / total)</div>
      ...
      {data?.email && (
        <section className="mb-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <FiMail className="text-[#00A676]" /> Eventos de entrega de email
            <span className="ml-2 text-sm font-normal text-gray-500">({data.email.recommendationEventsCount} total en el rango)</span>
          </h2>
          {data.email.recentEvents.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-xl p-6 text-center text-gray-400 text-sm">
              Sin eventos de email de recomendación en este período.
            </div>
          ) : (
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-gray-500 font-semibold">ID Usuario</th>
                    <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-gray-500 font-semibold">Tipo de evento</th>
                    <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-gray-500 font-semibold">Fecha/hora</th>
                  </tr>
                </thead>
      ...
      {data?.jobs?.recentFailures && data.jobs.recentFailures.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <FiAlertTriangle className="text-red-500" /> Fallos recientes de jobs
          </h2>
          <div className="bg-white border border-red-200 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-red-50 border-b border-red-200">
                <tr>
                    <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-red-600 font-semibold">ID Usuario</th>
                  <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-red-600 font-semibold">ID Búsqueda</th>
                  <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-red-600 font-semibold">Propiedades</th>
                  <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-red-600 font-semibold">Error</th>
                  <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-red-600 font-semibold">Hora</th>
                </tr>
              </thead>
      ...
      {!loading && data && totals && totals.total === 0 && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
          ...
          <p className="text-green-700 font-semibold text-lg">Sin jobs de recomendación encontrados</p>
          <p className="text-green-600 text-sm mt-1">El pipeline está inactivo para el rango de tiempo seleccionado. Los jobs se crean automáticamente cuando se publican propiedades.</p>
        </div>
      )}
    </div>
  )
